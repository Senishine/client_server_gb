...
def start_server(address='', port=7777):
    ...
    s.listen(5)
    s.setblocking(False)
    input_sockets = [s]  # sockets which should be checked for available data to read
    output_sockets = []  # sockets which should be checked for readiness to write data
    messages = {}
    ...
    def cleanup_socket(sck):
        ...
        del messages[sck]
        logger.info('Socket %s was closed, remaining: input=%s, output=%s, messages=%s',
